...
> Catalog pages under `collections/_catalog` are deleted and regenerated by `.github/workflows/update-catalog.yml`, so an edit to one does not survive the next scheduled run.

Names the tree, names the mechanism, and tells you exactly which pull request it rejects.

Note what it does not do: it does not sweep a second tree in with the first. Model pages under `collections/_models` are written by a different pipeline, in a different repository, with different behavior, so they need their own sentence with their own citation. Bundling trees together because they feel similar is how a vision ends up asserting something its evidence does not support.
